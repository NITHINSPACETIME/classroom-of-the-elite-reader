...
interface MushokuTenseiSelectClientProps {
  volumes: MushokuTenseiVolumeData[];
}

const mushokuTenseiBackgrounds = [
...
const getCompactChapterLabel = (title: string, index: number) => {
  ...
  if (lower.includes("extra chapter")) return "Extra Ch.";
...
export default function MushokuTenseiSelectClient({ volumes }: MushokuTenseiSelectClientProps) {
  ...
  const currentVolumesList = useMemo(() => {
    ...
    if (activeTab === "redundancy") return mushokuTenseiRedundancy;
    return mushokuTenseiSideStories;
  }, [activeTab, volumes]);
  ...
  return (
    <div className="min-h-screen w-full bg-[#050605] text-[#e2f9ec] overflow-y-auto relative flex flex-col items-center select-none">
      {/* Background Slideshow & Gradients */}
      <div className="absolute inset-0 z-0 select-none pointer-events-none fixed">
        <div className="absolute inset-0 bg-[#050605]/40 z-10" />
        <div className="absolute inset-0 bg-gradient-to-t from-[#050605] via-[#050605]/30 to-transparent z-10" />
        <BackgroundSlideshow images={mushokuTenseiBackgrounds} interval={8000} imageOpacity={0.7} />
      </div>
      ...
      {/* Top Header */}
      <header className="sticky top-0 left-0 w-full z-50 p-6 bg-gradient-to-b from-[#050605]/90 to-transparent backdrop-blur-md flex items-center justify-between border-b border-emerald-950/20">
        <div className="flex items-center">
          <Link href="/mushoku-tensei">
            <Button variant="ghost" size="icon" className="text-emerald-300 hover:text-white hover:bg-emerald-950/40 rounded-full transition-all">
          ...
          <h1 className="ml-4 text-sm sm:text-base md:text-lg lg:text-xl font-serif font-extralight tracking-[0.15em] uppercase text-zinc-100 truncate max-w-[200px] sm:max-w-none">
            Mushoku Tensei: <span className="text-emerald-355 font-normal">Jobless Reincarnation</span>
          </h1>
        </div>
      ...
      >
        ...
        <div className="flex flex-col items-center text-center mt-8 mb-12 gap-6">
          ...
          {/* Tabs Navigation */}
          {(mushokuTenseiRedundancy.length > 0 || mushokuTenseiSideStories.length > 0) && (
            <div className="flex justify-center mt-2">
              <div className="relative bg-emerald-950/20 backdrop-blur-md p-1.5 rounded-full border border-emerald-900/30 flex items-center gap-1">
                {/* Main Volumes */}
                <button
                  ...
                  className={`relative px-6 py-2 rounded-full text-xs font-bold font-mono tracking-widest uppercase transition-colors duration-300 z-10 cursor-pointer ${
                    activeTab === "main" ? "text-white" : "text-emerald-300/60 hover:text-emerald-250"
                  }`}
                >
                  {activeTab === "main" && (
                    <motion.div
                      layoutId="mushokuTenseiActiveTab"
                      className="absolute inset-0 bg-emerald-900/60 rounded-full shadow-[0_0_15px_rgba(16,185,129,0.3)] border border-emerald-500/30"
                  ...
                  <span className="relative z-10 flex items-center gap-2">
                    ...
                    <span className={`text-[10px] px-1.5 py-0.5 rounded-full ${
                      activeTab === "main" ? "bg-white/20 text-white" : "bg-emerald-950/40 text-emerald-400"
                    }`}>
                      {volumes.length}
                    </span>
                  </span>
                ...
                {/* Redundancy (After-Story) */}
                <button
                  onClick={() => setActiveTab("redundancy")}
                  className={`relative px-6 py-2 rounded-full text-xs font-bold font-mono tracking-widest uppercase transition-colors duration-300 z-10 cursor-pointer ${
                    activeTab === "redundancy" ? "text-white" : "text-emerald-300/60 hover:text-emerald-250"
                  }`}
                >
                  ...
                  <span className="relative z-10 flex items-center gap-2">
                    Redundancy (sequel)
                    <span className={`text-[10px] px-1.5 py-0.5 rounded-full ${
                      activeTab === "redundancy" ? "bg-white/20 text-white" : "bg-emerald-950/40 text-emerald-400"
                    }`}>
                      {mushokuTenseiRedundancy.length}
                    </span>
                  </span>
                </button>
                ...
                <button
                  ...
                  className={`relative px-6 py-2 rounded-full text-xs font-bold font-mono tracking-widest uppercase transition-colors duration-300 z-10 cursor-pointer ${
                    activeTab === "side" ? "text-white" : "text-emerald-300/60 hover:text-emerald-250"
                  }`}
                >
                  {activeTab === "side" && (
                    <motion.div
                  ...
                  <span className="relative z-10 flex items-center gap-2">
                    Side Stories
                    <span className={`text-[10px] px-1.5 py-0.5 rounded-full ${
                      activeTab === "side" ? "bg-white/20 text-white" : "bg-emerald-950/40 text-emerald-400"
                    }`}>
                      {mushokuTenseiSideStories.length}
                    </span>
                  </span>
                </button>
              </div>
          ...
          <div className="relative w-full max-w-md">
            <Search className="absolute left-3 top-3 h-4 w-4 text-emerald-400/60" />
            <input
              type="text"
              placeholder="Search volumes, synopses, chapters..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full rounded-full border border-emerald-900/30 bg-black/40 text-white placeholder:text-emerald-300/40 text-sm pl-10 pr-4 py-2.5 outline-none focus:ring-1 focus:ring-emerald-400 focus:border-emerald-400 transition-all shadow-[0_0_15px_rgba(16,185,129,0.02)] focus:shadow-[0_0_20px_rgba(16,185,129,0.1)]"
            />
          </div>
        </div>
          ...
          {viewMode === "detailed" ? (
            ...
            >
              {filteredVolumes.map((vol) => (
                <div
                  key={vol.id}
                  className="bg-black/45 border border-emerald-900/20 hover:border-emerald-500/30 rounded-2xl overflow-hidden shadow-2xl backdrop-blur-xl transition-all duration-300 relative"
                >
                  <div className="absolute top-0 left-0 right-0 h-[1px] bg-gradient-to-r from-transparent via-emerald-500/20 to-transparent" />
                  
                  {/* Info and cover grid */}
                  <div className="grid grid-cols-1 md:grid-cols-[1.5fr_250px] divide-y md:divide-y-0 md:divide-x divide-emerald-950/20">
                    
                    {/* Left Pane - Metadata and chapters list */}
                    <div className="p-6 md:p-8 flex flex-col justify-between gap-6">
                      <div className="flex flex-col gap-3">
                        <div className="flex items-center gap-2">
                          <span className="font-mono text-xs text-emerald-400 font-bold uppercase tracking-widest">Volume {vol.volumeNumber}</span>
                          {vol.tag && (
                            <span className="bg-emerald-950/60 text-emerald-300 border border-emerald-900/30 text-[10px] font-mono font-bold uppercase tracking-wider px-2.5 py-0.5 rounded-full shadow-[0_0_10px_rgba(16,185,129,0.05)]">
                              {vol.tag}
                            </span>
                          )}
                      ...
                      <div className="flex flex-wrap items-center gap-4 text-xs text-emerald-305 border-t border-emerald-950/40 pt-4">
                        <div className="flex items-center gap-1.5 bg-emerald-950/20 px-3 py-1 rounded-full border border-emerald-900/30">
              ...
              {filteredVolumes.map((vol) => (
                ...
                >
                  ...
                  <div className="text-center mt-1 px-1">
                    <div className="font-bold text-zinc-200 text-xs group-hover:text-emerald-300 transition-colors truncate">
                      {vol.title}
                    </div>
                    {vol.title.toLowerCase() !== `volume ${vol.volumeNumber}`.toLowerCase() && (
                      <div className="text-[10px] text-zinc-500 truncate group-hover:text-zinc-400 transition-colors">
                        {vol.volumeNumber.startsWith("Redundancy") || vol.volumeNumber.startsWith("SS") ? vol.volumeNumber : `Volume ${vol.volumeNumber}`}
                      </div>
                    )}
                  </div>
                </div>
