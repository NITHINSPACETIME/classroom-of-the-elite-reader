...
export function VolumePageClient({ volumeId }: { volumeId: string }) {
    ...
    useEffect(() => {
        ...
        const savedMeta = localStorage.getItem(`bunny-girl-progress-meta-${volId}`);
        const savedProgress = localStorage.getItem(`bunny-girl-progress-${volId}`);
        let chapIndex = 0;
        let scrollPct = 0;
        if (savedMeta) {
            try {
    ...
    const getContinueReadingUrl = () => {
        return `/bunny-girl/read/${volume.id}/${hasStarted ? getContinueChapterIndex() : 1}`;
    };

    return (
        <div className="min-h-screen w-full bg-[#0a0309] text-zinc-105 overflow-y-auto relative flex flex-col select-none theme-bunnygirl">
            <div className="absolute inset-0 z-0 select-none pointer-events-none fixed">
                <div className="absolute inset-0 bg-[#0a0309]/85 z-10" />
                <div className="absolute inset-0 bg-gradient-to-t from-[#0a0309] via-[#0a0309]/60 to-transparent z-10" />
                <BackgroundSlideshow
                    images={[
            ...
            <nav className="relative z-50 p-6 flex items-center justify-between border-b border-purple-950/20 bg-gradient-to-b from-[#0a0309]/90 to-transparent backdrop-blur-md">
                <Link href="/bunny-girl/select">
                    <Button variant="ghost" className="text-zinc-400 hover:text-white hover:bg-purple-950/30 rounded-full transition-all gap-2">
                        <ArrowLeft className="w-5 h-5" />
                        Back to Select
                    </Button>
                </Link>
                <h1 className="text-2xl font-serif font-bold text-white tracking-widest hidden sm:block">Mai Sakurajima Archive</h1>
                <UserMenu
            ...
            <main className="relative z-10 container mx-auto px-4 lg:px-8 pb-20 pt-6 max-w-5xl flex-1">
                <div className="lg:hidden mb-6 space-y-2">
                    ...
                    <div className="flex flex-wrap gap-2 pt-2 text-xs text-purple-400 font-mono uppercase tracking-wider">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-[300px_1fr] gap-8 lg:gap-12 items-start">
                    ...
                    >
                        <div className="relative aspect-[2/3] w-full max-w-[240px] lg:max-w-none mx-auto lg:mx-0 rounded-2xl overflow-hidden shadow-[0_0_50px_rgba(217,70,239,0.15)] border border-purple-900/30 group">
                        ...
                        <div className="w-full flex flex-col gap-3">
                            {volume.inProgress ? (
                                ...
                                <Link href={getContinueReadingUrl()} className="w-full">
                                    <Button className="w-full bg-purple-700 hover:bg-purple-600 text-white font-bold font-serif py-6 tracking-widest text-xs uppercase shadow-[0_0_15px_rgba(217,70,239,0.3)] rounded-full">
                            ...
                            <Button
                                ...
                                className="w-full border-purple-950/30 hover:bg-purple-950/20 text-zinc-400 hover:text-white text-xs py-5 rounded-full transition-all gap-2"
                            >
                            ...
                            {hasStarted && (
                                <div className="w-full bg-purple-950/10 border border-purple-900/20 rounded-2xl p-4 text-center mt-2">
                                    <div className="text-[10px] text-purple-400 font-bold tracking-wider uppercase mb-1">Last Read</div>
                    ...
                    >
                        <div className="hidden lg:block space-y-2 border-b border-purple-950/20 pb-4">
                            <h2 className="text-4xl font-extrabold font-serif tracking-tight text-white animate-fadeIn">
                                {volume.title}
                            </h2>
                            <div className="flex items-center gap-3 text-xs text-purple-400 font-mono uppercase tracking-wider pt-1">
                                <span>Volume {volume.volumeNumber}</span>
                                {volume.tag && (
                                    <>
                        ...
                        {/* Synopsis */}
                        <div>
                            <h3 className="text-xs font-bold uppercase tracking-widest text-purple-400 mb-3">Volume Synopsis</h3>
                            <p className="text-sm text-zinc-300 leading-relaxed font-sans bg-purple-950/5 border border-purple-950/10 p-5 rounded-2xl">
                                {volume.synopsis}
                            </p>
                        </div>

                        {/* Key Dates / Details */}
                        <div className="grid grid-cols-2 gap-4 text-xs bg-black/35 p-3 rounded-xl border border-purple-950/20 max-w-md">
                            <div>
                        ...
                        <div className="space-y-4">
                            <div className="flex flex-col sm:flex-row gap-3 items-center justify-between border-b border-purple-950/10 pb-4">
                                <h3 className="text-xs font-bold uppercase tracking-widest text-purple-400 self-start sm:self-center">
                                ...
                                <div className="flex gap-2 w-full sm:w-auto">
                                    <div className="relative flex-1 sm:w-64">
                                        ...
                                        <Input
                                            ...
                                            className="pl-9 bg-black/40 border-purple-950/40 text-zinc-200 focus:border-purple-500/50 rounded-full"
                                        />
                                    </div>
                                    <Button
                                        variant="outline"
                                        ...
                                        className="border-purple-950/40 hover:bg-purple-950/20 text-zinc-400 hover:text-white rounded-full"
                                    >
                            ...
                            <div className="space-y-2 max-h-[500px] overflow-y-auto pr-2 custom-scrollbar">
                                {sortedChapters.map((chap) => {
                                    ...
                                    return (
                                        <div
                                            key={originalIndex}
                                            className={`flex items-center justify-between p-4 rounded-xl border border-white/5 bg-zinc-950/30 hover:bg-purple-950/10 hover:border-purple-500/30 transition-all duration-300 w-full group ${isRead ? 'opacity-60 hover:opacity-100 transition-opacity' : ''}`}
                                        >
                                            <Link
                                                href={`/bunny-girl/read/${volume.id}/${originalIndex + 1}`}
                                                className={`text-sm text-zinc-350 group-hover:text-purple-400 transition-colors font-medium flex-1`}
                                            >
                                                {chap}
                                            </Link>
                                ...
                                {sortedChapters.length === 0 && (
                                    <div className="text-center py-12 text-zinc-550 border border-dashed border-purple-950/20 rounded-2xl">
            ...
            {hasStarted && (
                ...
                >
                    <a href={`/bunny-girl/read/${volume.id}/${getContinueChapterIndex()}`} className="pointer-events-auto">
                        <Button className="h-14 px-5 md:px-8 rounded-full bg-white/10 backdrop-blur-xl border border-white/20 text-white font-medium shadow-[0_8px_32px_rgba(0,0,0,0.5)] hover:bg-white/20 hover:scale-105 transition-all duration-300 group max-w-[calc(100vw-9.5rem)] md:max-w-md">
                            <BookOpen className="mr-2 w-5 h-5 text-purple-400 group-hover:text-purple-300 shrink-0" />
